...
DEMO_MODE = os.environ.get("DEMO_MODE", "1").strip().lower() in (
    "1", "true", "yes", "on"
)
...
# JSON Error Helper
# -----------------------------
def json_error(code: str, message: str, status: int = 400, hint: str = None):
...
def heuristic_score(text: str, evidence: str, policy_mode: str):
    ...
    volatile_triggers = (
        "ceo", "current", "today", "right now", "as of",
        "latest", "president", "prime minister"
    )

    is_volatile = any(k in tt for k in volatile_triggers)
    ...
    if any(term in tt for term in legal_authority_terms):
        is_volatile = True
    ...
    explanation = (
        "MVP heuristic scoring with volatility + evidence gating. "
...
def _volatility_category(text_lc: str) -> str:
    """
    Demo-friendly category labeling for VC signaling.
    Returns: MEDICAL | LEGAL | FINANCIAL | NEWS | GENERAL
    """
    t = (text_lc or "").lower()

    if any(k in t for k in ["diagnosis", "treatment", "medication", "dose", "dosage", "clinical", "patient"]):
        return "MEDICAL"

    if any(k in t for k in ["court", "statute", "legal", "regulation", "lawsuit", "precedent", "filing"]):
        return "LEGAL"

    if any(k in t for k in ["earnings", "revenue", "stock", "market", "ipo", "sec", "quarter", "guidance"]):
        return "FINANCIAL"

    if any(k in t for k in ["resigned", "announced", "today", "yesterday", "breaking", "appointed", "acquired"]):
        return "NEWS"
...
# -----------------------------
# DEMO response shaping (single canonical)
# -----------------------------
def shape_demo_response(resp_obj: dict) -> dict:
    ...
    if isinstance(raw_decision, dict):
        ...
        decision_obj = {
            ...
            "reason": (resp_obj.get("decision_detail") or {}).get("reason", "") or "",
        }
    ...
    shaped = {
        "contract": {
            ...
            "request_id": event_id,  # ✅ never blank in demo
        },
...
def api_score():
    try:
        ...
        # Fingerprint / Event ID (demo-friendly deterministic)
        ...
        # ✅ Fill volatility_category everywhere (top-level + signals)
        text_lc = text.lower()
        vcat = _volatility_category(text_lc)
        signals["volatility_category"] = vcat
        ...
        # ✅ Latency realism: never 0ms in demo
        latency_ms = max(5, int((time.time() - start) * 1000))

        resp_obj = {
            "schema_version": SCHEMA_VERSION,

            # ✅ VC polish: request_id never blank
            "request_id": event_id,
            ...
            # ✅ optional top-level echo for UI payload
            "volatility": signals.get("volatility"),
            "volatility_category": vcat,
